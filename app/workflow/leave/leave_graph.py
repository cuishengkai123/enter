...
def extract_slots_node(state: LeaveState) -> dict:
    ...
    # 优先使用 state 中的 requester，确保不会变成 anonymous
    req["requester"] = state.get("requester") or req.get("requester") or "peter"
    return {"req": req}


def validate_node(state: LeaveState) -> dict:
    req = state.get("req") or {}
    # 优先使用 state 中的 requester，如果没有再使用 req 中的，最后使用默认值
    requester = state.get("requester") or req.get("requester") or "peter"

    print(f"\n🔍 [VALIDATE] 开始验证 - requester={requester}")
    
    bal = get_leave_balance(requester) or {}
    print(f"[VALIDATE] 数据库返回的余额：{bal}")
    
    annual_balance = float(bal.get("annual_days", 0)) if bal else 0.0
    print(f"[VALIDATE] 提取的年假余额：{annual_balance}")

    missing, violations = validate_leave(req, balances_days=annual_balance)
    print(f"[VALIDATE] 验证结果 - missing:{missing}, violations:{violations}")
